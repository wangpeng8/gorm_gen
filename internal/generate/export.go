...
import (
	...
	"github.com/wangpeng8/gorm_gen/field"
	"github.com/wangpeng8/gorm_gen/helper"
	"github.com/wangpeng8/gorm_gen/internal/model"
	"github.com/wangpeng8/gorm_gen/internal/parser"
)
...
func GetQueryStructMeta(db *gorm.DB, conf *model.Config) (*QueryStructMeta, error) {
	...
	if conf.ModelNameNS == nil {
		conf.ModelNameNS = CreateModelNs
	}
	...
	return AppendPluginsImportPkgPaths(&QueryStructMeta{
		db:              db,
		Source:          model.Table,
		Generated:       true,
		FileName:        fileName,
...
// CreateModelNs 生成model name
func CreateModelNs(tableName string) (modelName string) {
	if tableName == "" {
		return ""
	}
	var (
	...
	for i := 0; i < count; i++ {
		v := src[i]
		if v != '_' {
			values = append(values, strings.ToLower(string([]rune{v})))
		}
		if v == '_' && i < count {
			values = append(values, strings.ToUpper(string([]rune{src[i+1]})))
			i++
		}
	}
	values[0] = strings.ToUpper(values[0])
