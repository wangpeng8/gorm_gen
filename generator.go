...
import (
	...
	"github.com/wangpeng8/gorm_gen/internal/generate"
	"github.com/wangpeng8/gorm_gen/internal/model"
	"github.com/wangpeng8/gorm_gen/internal/parser"
	tmpl "github.com/wangpeng8/gorm_gen/internal/template"
	"github.com/wangpeng8/gorm_gen/internal/utils/pools"
)
...
func (g *Generator) generateModelFile() error {
	...
	for _, data := range models {
		...
		go func(data *generate.QueryStructMeta) {
			...
			var buf bytes.Buffer
			modelTpl := tmpl.Model
			err := render(modelTpl, &buf, data)
			if err != nil {
				errChan <- err
				return
			}
			methodTpl := tmpl.ModelMethod
			for _, method := range data.ModelMethods {
				err = render(methodTpl, &buf, method)
				if err != nil {
					errChan <- err
					return
				}
